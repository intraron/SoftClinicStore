...
<div class="container">
    ...
    <c:url var="Url" value="/registrarpaciente.html" />
            ...
            <table width="100%" border="0">
                ...
                <tr>
                  ...
                  <div class="input-group date">
                    ...
                    <input name="dtFechaNacimiento" id="datepicker" type='text' class="form-control" placeholder="Seleccione su fecha de nacimiento" aria-describedby="fnacimiento_id" readonly>
                ...
                <tr>
                  <td width="47,5%">
                    <div class="form-group">
						...
						<textarea class="form-control" rows="5" id="sPuntodeReferencia" name="sPuntodeReferencia" placeholder="Indique un punto de referencia cercano"></textarea>
					</div>
                  </td>
                  <td width="5%">&nbsp;</td>
                  <td width="47,5%">&nbsp;</td>
    ...
    <p>
        ...
        <a href="irbandeja.html" id="cancel" name="cancel" class="btn btn-danger btn-lg">Cancelar</a>
    </p>
    </div>
		...
		$("#aceptarregistro").click(function(e) {
			...
			if ($("#sPrimerNombre").val().trim() === '' || $("#sPrimerApellido").val().trim() === '' || $("#sNombredeEmpresa").val().trim() === '' || 
				...
				$("#sendFromModal").dialog({
					buttons : {
						"Ok" : function() {
							if($("#sUsuarioSis").val().trim() === '')user_ckeck=true;
							if(user_ckeck===true){
								$("#registroform").submit();	
							}
							else{
								$("#uniqueUserModal").dialog({
									buttons : {
										"Ok" : function() {
											$(this).dialog("close");
											$("#sUsuarioSis").focus();
				...
				/*Estos mensajes propios de js son bien ligeros y no hay que hacer mayor configuracion si no llamarlos para 
				  implementarlos pero estos usando los mensajes de jquery-ui 
				  
				  var r = confirm("¿Desea guardar el registro del Paciente?"); //Devuelve true o false implementa dos botones
				  alert("Modifique el Usuario de Sistema ingresado, No está disponible."); // solo implementa boton ok
				 */
			}
		});
	});	
